...
await query.edit_message_text(
    ...
    "🔴 Qarzdor  ⚪ Hisob-kitob  🟢 Ortiqcha to'lagan",
    parse_mode='Markdown',
    reply_markup=InlineKeyboardMarkup(keyboard)
)
```
...
if debt > 0:
    ...
    status = f"🟢 Ortiqcha to'lagan: *{abs(debt):,.0f} so'm*"
else:
    status = "✅ Hisob-kitob qilingan"

keyboard = [
    [InlineKeyboardButton("➕ Qarz qo'shish", callback_data=f"add_debt_{client_id}")],
    [InlineKeyboardButton("➖ Qarz ayirish (to'lov)", callback_data=f"pay_debt_{client_id}")],
    [InlineKeyboardButton("📜 Tarix", callback_data=f"history_{client_id}")],
    [InlineKeyboardButton("🗑 Mijozni o'chirish", callback_data=f"delete_client_{client_id}")],
    [InlineKeyboardButton("◀️ Orqaga", callback_data="clients_list")],
]
...
keyboard = [[InlineKeyboardButton("◀️ Orqaga", callback_data=f"client_{client_id}")]]
await query.edit_message_text(
...
try:
    ...
    await update.message.reply_text(
        "❌ Noto'g'ri miqdor! Faqat son kiriting:\nMasalan: `150000`",
        parse_mode='Markdown'
    )
    return WAITING_DEBT_AMOUNT
...
async def process_transaction(update: Update, context: ContextTypes.DEFAULT_TYPE, note: str):
...
conn.execute(
    "INSERT INTO transactions (client_id, amount, type, note) VALUES (?,?,?,?)",
...
async def process_transaction_query(query, context, note: str):
client_id = context.user_data.get(‘current_client_id’)
amount = context.user_data.get(‘debt_amount’)
action = context.user_data.get(‘debt_action’)
...
if new_debt > 0:
    ...
    balance_text = f"🟢 Ortiqcha to'lagan: *{abs(new_debt):,.0f} so'm*"
else:
...
debtors = conn.execute(
    "SELECT * FROM clients WHERE total_debt > 0 ORDER BY total_debt DESC"
).fetchall()
conn.close()
...
conn = get_db()
top = conn.execute(
    "SELECT * FROM clients WHERE total_debt > 0 ORDER BY total_debt DESC LIMIT 10"
).fetchall()
conn.close()
...
keyboard = [
    [InlineKeyboardButton("◀️ Orqaga", callback_data="statistics")],
    [InlineKeyboardButton("🏠 Asosiy menyu", callback_data="main_menu")],
]
await query.edit_message_text(text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(keyboard))
```
...
await query.edit_message_text(
    f"⚠️ *{client['name']}* ni o'chirishni tasdiqlaysizmi?\n\nBarcha tarix ham o'chib ketadi!",
    parse_mode='Markdown',
    reply_markup=InlineKeyboardMarkup(keyboard)
)
```
...
keyboard = [[InlineKeyboardButton("🏠 Asosiy menyu", callback_data="main_menu")]]
await update.message.reply_text(
    "❓ Noma'lum buyruq. /start yuboring yoki menyudan foydalaning.",
    reply_markup=InlineKeyboardMarkup(keyboard)
)
```
...
add_client_conv = ConversationHandler(
    ...
    states={
        WAITING_CLIENT_NAME: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, save_client)
        ],
    },
...
# Conversation handler - qarz qo'shish/ayirish
